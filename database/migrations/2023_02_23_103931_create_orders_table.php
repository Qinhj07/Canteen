...
class CreateOrdersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('orders', function (Blueprint $table) {
            $table->id();
            $table->string('oid', 32)->comment('订单号');
            $table->string('code', 32)->comment('用餐码')->unique();
            $table->string('openid', 32)->comment('openid');
            $table->string('phone', 15)->comment('下单号码');
            $table->string('receipt_id', 32)->comment('餐码, 关联餐品信息');
