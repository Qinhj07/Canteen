...
class CreatePayOrdersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('pay_orders', function (Blueprint $table) {
            $table->id();
            $table->string('order_id', 32)->comment('订单号')->unique();
            ...
            $table->string('check_money')->comment('接口返回金额/分')->nullable();
            $table->json('items')->comment('订餐内容备份');
            $table->timestamp('check_at')->comment('接口返回支付时间')->nullable();
            $table->timestamps();
